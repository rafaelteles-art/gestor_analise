...
describe('enqueueCampaignJobs — broadcast is all-or-nothing (review fix)', () => {
  // The transactional enqueue lives in lib/campaign-jobs.ts, which pulls in
  // meta-campaigns.ts (whose `@/` aliases vitest can't resolve), so we assert the
  // structural guarantee on the source text — a REGRESSION GUARD: if someone
  // reverts the per-account INSERT loop back to N independent pool.query() calls
  // with no surrounding transaction, a mid-loop DB blip would commit the earlier
  // accounts' jobs (which then RUN) while returning 500, and a user retry would
  // double-enqueue the already-committed accounts (double-spend). These fail then.
  const src = readFileSync(
    join(dirname(fileURLToPath(import.meta.url)), '..', 'campaign-jobs.ts'),
    'utf8'
  );

  function enqueueBody(): string {
    const start = src.indexOf('export async function enqueueCampaignJobs');
    const end = src.indexOf('export async function claimNextCampaignJob');
    expect(start).toBeGreaterThan(-1);
    expect(end).toBeGreaterThan(start);
    return src.slice(start, end);
  }

  it('wraps the per-account INSERTs in a single BEGIN/COMMIT transaction', () => {
  ...
  it('rolls back and releases the client if any INSERT throws mid-loop', () => {
    const body = enqueueBody();
    expect(body).toContain("client.query('ROLLBACK')");
    expect(body).toContain('client.release()');
  });

  it('inserts through the transactional client, not the raw pool, inside the loop', () => {
    const body = enqueueBody();
    // The INSERT must run on the leased client (so it participates in the tx), not
    // pool.query (which would auto-commit each row outside the transaction).
    expect(body).toContain('client.query(');
    expect(body).toContain('INSERT INTO campaign_jobs');
    // The only acceptable use of the raw pool here is pool.connect(); a direct
    // pool.query INSERT would commit outside the transaction.
    expect(body).not.toContain('pool.query(');
  });
});

describe('media checkpoint decode — vid thumbnail split on FIRST | only (review fix)', () => {
  // applyMediaCheckpoint is a non-exported helper in lib/campaign-jobs.ts (which
  // pulls in meta-campaigns.ts), so we assert the decode logic on the source text.
  // REGRESSION GUARD: the original `rest.split('|')` silently dropped everything
  // after the first delimiter, corrupting a thumbnail URL that contains a '|'.
  const src = readFileSync(
    join(dirname(fileURLToPath(import.meta.url)), '..', 'campaign-jobs.ts'),
    'utf8'
  );
  ...
  function applyBody(): string {
    const start = src.indexOf('function applyMediaCheckpoint');
    expect(start).toBeGreaterThan(-1);
    const end = src.indexOf('function currentToken', start);
    expect(end).toBeGreaterThan(start);
    return src.slice(start, end);
  }

  it('does NOT decode the vid checkpoint with the lossy rest.split(\'|\')', () => {
    expect(applyBody()).not.toContain("rest.split('|')");
  });

  it('splits the vid checkpoint on the FIRST | via indexOf/slice', () => {
    const body = applyBody();
    expect(body).toContain("indexOf('|')");
    // video_id = before the first '|', thumb = everything after it (may contain '|').
    expect(body).toContain('rest.slice(sep + 1)');
  });
});
