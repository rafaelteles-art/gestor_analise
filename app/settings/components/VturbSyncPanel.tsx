...
export default function VturbSyncPanel() {
  ...
  return (
    <div className="bg-console-surface border border-console-border rounded p-6">
      <div className="mb-4">
        <h2 className="text-sm font-bold text-foreground">Importar histórico — vturb Analytics</h2>
        <p className="text-xs text-console-muted mt-1">
          Busca dados diários de todos os players via API oficial do vturb e grava em <code className="text-[10px] px-1 py-0.5 bg-console-surface-2 rounded text-foreground">vturb_metrics</code>.
        </p>
      </div>
            ...
            {([7, 14, 30, 60, 90] as const).map(d => (
              <button key={d} onClick={() => setPeriod(d)} disabled={running}
                className={`px-3 py-1 text-xs font-semibold rounded transition-colors disabled:opacity-40 ${
                  period === d ? 'bg-fuchsia-600 text-white' : 'border border-console-border text-foreground hover:bg-console-surface-2'
                }`}
              >{d}d</button>
            ))}
            <button onClick={() => setPeriod('today')} disabled={running}
              className={`px-3 py-1 text-xs font-semibold rounded transition-colors disabled:opacity-40 ${
                period === 'today' ? 'bg-fuchsia-600 text-white' : 'border border-console-border text-foreground hover:bg-console-surface-2'
              }`}
            >Hoje</button>
            <button onClick={() => setPeriod('yesterday')} disabled={running}
              className={`px-3 py-1 text-xs font-semibold rounded transition-colors disabled:opacity-40 ${
                period === 'yesterday' ? 'bg-fuchsia-600 text-white' : 'border border-console-border text-foreground hover:bg-console-surface-2'
              }`}
            >Ontem</button>
            <button onClick={() => setPeriod('range')} disabled={running}
              className={`px-3 py-1 text-xs font-semibold rounded transition-colors disabled:opacity-40 ${
                period === 'range' ? 'bg-fuchsia-600 text-white' : 'border border-console-border text-foreground hover:bg-console-surface-2'
              }`}
            >Datas específicas</button>
          </div>
          <button onClick={handleSync} disabled={running || !rangeValid}
            className="ml-auto flex items-center gap-2 px-4 py-2 text-xs font-semibold rounded bg-fuchsia-600 text-white hover:bg-fuchsia-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {running
              ? <><SpinIcon /> Importando...</>
              : <><UploadIcon /> {
          ...
          <div className="flex items-center gap-3 bg-console-surface-2 border border-console-border rounded px-3 py-2 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <input type="date" value={rangeFrom} max={todayStr}
                ...
                className="text-xs px-2 py-1 rounded border border-console-border bg-background text-foreground focus:border-fuchsia-500 focus:ring-1 focus:ring-fuchsia-500 outline-none disabled:opacity-50"
              />
            </div>
            <div className="flex items-center gap-2">
