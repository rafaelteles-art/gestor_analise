...
export default function ClientImport({ dbAccounts, rtCampaigns, offers, currentOferta }: ClientImportProps) {
  ...
  useEffect(() => {
    ...
    // Semântica do single-select RT: primeira campanha escopada (ou '').
    ...
    try {
        ...
        if (savedStr) {
            ...
            // Só restaura a campanha RT armazenada se ela existir na lista escopada;
            // caso contrário mantém o default (primeira campanha / '').
        ...
        <div key={accId} className="bg-white dark:bg-gray-900 border text-gray-800 dark:text-gray-100 border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="divide-y divide-gray-100 dark:divide-gray-800">
            {accSorted.map((group: any) => {
                ...
                const roasColor = group.roas >= 1 ? 'text-emerald-500' : group.roas > 0 ? 'text-amber-500' : 'text-gray-400 dark:text-gray-500';
                    ...
                    >
                        <div className="col-span-2 px-6 py-3.5 flex items-center gap-3">
                            <div className="flex items-center justify-center w-8 cursor-pointer" onClick={() => toggleRow(expandKey)}>
                                {/* Toggle Arrow */}
                                <div className={`w-5 h-5 flex items-center justify-center rounded bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500 group-hover:bg-indigo-50 dark:group-hover:bg-indigo-950/40 group-hover:text-indigo-600 transition-colors ${isOpen ? 'rotate-90' : ''}`}>
                            ...
                            <span
                                className="font-bold text-gray-800 dark:text-gray-100 cursor-help"
                            ...
                            {group.meta_campaigns.length > 0 && (
                                <span className="text-[10px] bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 px-1.5 py-0.5 rounded-full font-medium">
                        ...
                        <div className="px-4 py-3.5 text-right font-mono text-gray-500 dark:text-gray-400">{group.meta_cpm > 0 ? formatCurrency(group.meta_cpm) : '—'}</div>
                        <div className="px-4 py-3.5 text-right font-mono text-gray-500 dark:text-gray-400">{group.meta_ctr > 0 ? formatPercent(group.meta_ctr) : '—'}</div>
          ...
          return 0;
                        ...
                        return (
                            ...
                            <div className="px-4 py-3 text-right font-mono text-gray-400 dark:text-gray-500">{formatCurrency(mc.cpm)}</div>
                            <div className="px-4 py-3 text-right font-mono text-gray-400 dark:text-gray-500">{formatPercent(mc.ctr)}</div>
